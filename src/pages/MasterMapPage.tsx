...
function brushPointsBetween(previous: FogPoint, next: FogPoint, radiusX: number, radiusY: number) {
  const relativeDistance = Math.max(Math.abs(next.x - previous.x) / Math.max(radiusX, 0.0005), Math.abs(next.y - previous.y) / Math.max(radiusY, 0.0005));
  return relativeDistance < 0.18 ? [] : [next];
}
...
export default function MasterMapPage() {
  ...
  function moveDrawing(event: PointerEvent<HTMLDivElement>) {
    ...
    if (isRectangle()) previewRef.current = rectangleBetween(dragStart, point, drawingMode());
      ...
      const additions = brushPointsBetween(previous, point, current.radiusX, current.radiusY);
      if (!additions.length) return;
      previewRef.current = { ...current, points: [...current.points, ...additions].slice(0, 1200) };
    }
    setPreview(previewRef.current);
  }
  ...
  const handleMetrics = useCallback((metrics: MapImageMetrics) => {
    if (!autoFit) return;
  ...
  const selectPin = useCallback((pin: MapPin) => setPinDraft(pin), []);
  const selectMonster = useCallback((monster: MapMonster) => setMonsterDraft(monster), []);
